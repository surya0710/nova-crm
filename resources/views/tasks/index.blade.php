...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-lg font-semibold text-slate-900">{{ __('Tasks') }}</h1>
                <p class="text-sm text-slate-500">{{ __('Track follow-ups and to-dos') }}</p>
            </div>
            @can('create', App\Models\Task::class)
                <a href="{{ route('tasks.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700 transition shrink-0">
    ...
    <div class="rounded-xl bg-white border border-slate-200 shadow-sm p-4 sm:p-5 mb-6">
        <form method="GET" action="{{ route('tasks.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
            ...
            <select name="assigned_to" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
            ...
            <div class="flex gap-2">
                <select name="filter" class="flex-1 border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm">
    ...
    <div class="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
        @if ($tasks->isEmpty())
            <div class="p-12 text-center text-sm text-slate-500">{{ __('No tasks found.') }}</div>
        @else
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($tasks as $task)
                            <tr class="hover:bg-slate-50/50">
                                <td class="px-6 py-4">
                                    <a href="{{ route('tasks.show', $task) }}" class="font-medium text-indigo-600 hover:text-indigo-800">{{ $task->title }}</a>
                                    @if ($task->taskable)
                                ...
                                <td class="px-6 py-4 text-right">
                                    @can('update', $task)
                                        <a href="{{ route('tasks.edit', $task) }}" class="text-sm text-slate-500 hover:text-indigo-600">{{ __('Edit') }}</a>
                                    @endcan
                                </td>
                            </tr>
                        @endforeach
