<x-app-layout>
    ...
    <x-layouts.entity-listing
        :title="__('Employee Appraisals')"
    >
        <x-slot:breadcrumbs>
            <x-nav.breadcrumbs :items="[
                ['label' => __('HR'), 'href' => route('hrms.home')],
                ['label' => __('Performance'), 'href' => route('hrms.performance.index')],
                ['label' => __('Employee Appraisals'), 'current' => true],
            ]" />
        </x-slot:breadcrumbs>
        ...
        <table class="min-w-full text-sm">
            ...
            @forelse ($appraisals as $appraisal)
                <tr class="border-t">
                    ...
                    <td class="p-3">{{ $appraisal->final_rating ?? $appraisal->manager_rating ?? '—' }}</td>
            ...
            @empty
                <tr><td colspan="5" class="p-6 text-center text-slate-500">{{ __('No appraisals found.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
