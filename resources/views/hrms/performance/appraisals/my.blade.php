        ...
        <table class="min-w-full text-sm">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="p-3 text-left">{{ __('Closed') }}</th>
            ...
            @forelse ($appraisals as $appraisal)
                <tr class="border-t">
                    ...
                    <td class="p-3">{{ $appraisal->closed_at?->format('Y-m-d') ?? '—' }}</td>
            ...
            @empty
                <tr><td colspan="4" class="p-6 text-center text-slate-500">{{ __('No finalized appraisals available.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
