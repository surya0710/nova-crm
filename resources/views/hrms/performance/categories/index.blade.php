<x-app-layout>
    ...
    <x-layouts.entity-listing
        :title="__('Competency Categories')"
    >
        <x-slot:breadcrumbs>
            <x-nav.breadcrumbs :items="[
                ['label' => __('HR'), 'href' => route('hrms.home')],
                ['label' => __('Performance'), 'href' => route('hrms.performance.index')],
                ['label' => __('Competency Categories'), 'current' => true],
            ]" />
        </x-slot:breadcrumbs>

        @can('create', \App\Models\CompetencyCategory::class)
    <div class="rounded-xl border border-line bg-surface-card shadow-sm p-4 mb-6">
        <form method="POST" action="{{ route('hrms.performance.categories.store') }}" class="grid grid-cols-1 md:grid-cols-4 gap-3">
            @csrf
        ...
        <table class="min-w-full text-sm">
            ...
            @foreach ($categories as $category)
                <tr class="border-t">
                    ...
                    <td class="p-3">{{ $category->competencies_count }}</td>
                    <td class="p-3">
                        @can('delete', $category)
                        <form method="POST" action="{{ route('hrms.performance.categories.destroy', $category) }}">@csrf @method('DELETE') <button class="text-red-600">{{ __('Delete') }}</button></form>
                        @endcan
                    </td>
                </tr>
            @endforeach
