    ...
    >
        <x-slot:breadcrumbs>
            <x-nav.breadcrumbs :items="[
                ...
                ['label' => __('Feedback Requests'), 'current' => true],
            ]" />
        </x-slot:breadcrumbs>
        ...
        <table class="min-w-full text-sm">
            ...
            @forelse ($requests as $req)
                <tr class="border-t">
                    ...
                    <td class="p-3">
                        <a class="text-sm font-medium text-primary-700 hover:text-primary-800" href="{{ route('hrms.performance.feedback.requests.show', $req) }}">{{ __('View') }}</a>
                    </td>
                </tr>
            @empty
                <tr><td colspan="6" class="p-6 text-center text-slate-500">{{ __('No feedback requests.') }}</td></tr>
            @endforelse
            </tbody>
        </table>
