<x-app-layout>
    ...
    >
        ...
        <x-slot:actions>
            <x-ui.button :href="route('leads.import.create')" variant="secondary" size="sm">
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                @if (! empty($preview->errors))
                    <x-ui.button :href="route('leads.import.report', $session)" variant="secondary" size="sm">
                        {{ __('Download Validation Report') }}
                    </x-ui.button>
                    <x-ui.button :href="route('leads.import.report.xlsx', $session)" variant="secondary" size="sm">
                        {{ __('Download Validation Report (Excel)') }}
                    </x-ui.button>
                @endif

                @if ($preview->invalidRows > 0 || ! empty($preview->errors))
                    <x-ui.button :href="route('leads.import.errors', $session)" variant="link" size="sm">
                        {{ __('Download error report') }}
                    </x-ui.button>
                @endif

                <x-ui.button :href="route('leads.index')" variant="link" size="sm">{{ __('Cancel') }}</x-ui.button>
            </div>
        </div>
    </x-layouts.entity-listing>
</x-app-layout>
