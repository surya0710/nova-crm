    ...
    <div class="max-w-3xl space-y-6">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="mt-6 flex flex-wrap gap-3">
                <a href="{{ route('leads.index') }}" class="inline-flex items-center rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">
                    {{ __('View :leads', ['leads' => crm_term('leads')]) }}
                </a>
                <a href="{{ route('leads.import.create') }}" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    {{ __('Import another file') }}
                </a>
                @if ($session->failed_count > 0 || $duplicateRows > 0)
                    <a href="{{ route('leads.import.errors', $session) }}" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
