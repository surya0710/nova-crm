<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <a href="{{ route('customers.import.create') }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            @if (! empty($preview->errors))
                <a href="{{ route('customers.import.report', $session) }}" class="inline-flex items-center rounded-lg border border-indigo-300 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100">
                    {{ __('Download Validation Report') }}
                </a>
                <a href="{{ route('customers.import.report.xlsx', $session) }}" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                    {{ __('Download Validation Report (Excel)') }}
                </a>
            @endif

            @if ($preview->invalidRows > 0 || ! empty($preview->errors))
                <a href="{{ route('customers.import.errors', $session) }}" class="text-sm text-slate-600 hover:text-slate-800">
