                    ...
                    @if (! empty($item['children']))
                        <div x-data="{ open: {{ ($item['open'] ?? false) ? 'true' : 'false' }} }">
                            ...
                            >
                                ...
                                @endif
                                <span @class(['flex-1 text-left', 'sr-only' => $collapsed])>{{ $item['label'] }}</span>
                                @unless ($collapsed)
                                    <svg class="h-4 w-4 transition" :class="open ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5l7 7-7 7"/></svg>
                                @endunless
                            </button>
                            <div x-show="open" x-cloak class="ml-4 space-y-0.5 border-l border-sidebar-border pl-2">
