<x-app-layout>
    ...
    <x-layouts.edit :title="__('Edit Calendars')" max-width="4xl">
        <x-slot:breadcrumbs>
            <x-nav.breadcrumbs :items="[
                ['label' => __('Projects'), 'href' => route('projects.home')],
                ['label' => __('Edit Calendars'), 'current' => true],
            ]" />
        </x-slot:breadcrumbs>


    <form method="POST" action="{{ route('resources.calendars.update', $calendar) }}" class="max-w-3xl">
        @csrf
        @method('PUT')
