<x-app-layout>
    <x-slot name="header">
        <div>
            <h1 class="text-lg font-semibold text-slate-900">{{ __('Add Customer') }}</h1>
            <p class="text-sm text-slate-500">{{ __('Create a new customer account') }}</p>
        </div>
    </x-slot>
    ...
    <form method="POST" action="{{ route('customers.store') }}" class="max-w-4xl">
        ...
        <div class="rounded-xl bg-white border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="px-6 sm:px-8 py-4 border-t border-slate-200 bg-slate-50/50 flex items-center justify-between gap-4">
                <a href="{{ route('customers.index') }}" class="text-sm text-slate-600 hover:text-slate-900">{{ __('Cancel') }}</a>
