...
class ProjectStatusService
{
     ...
     */
    public function create(Organization $organization, array $data): ProjectStatus
    {
    ...
    public function delete(ProjectStatus $status): void
    {
        if ($status->projects()->exists()) {
            throw ValidationException::withMessages([
                'status' => __('Cannot delete a status that is used by projects.'),
            ]);
        }

        if ($status->is_default) {
            throw ValidationException::withMessages([
                'status' => __('The default project status cannot be deleted.'),
            ]);
        }

        $openStatuses = ProjectStatus::query()
        ...
        if (! $status->is_closed && $openStatuses === 0) {
            throw ValidationException::withMessages([
                'status' => __('At least one open project status must remain.'),
            ]);
        }
    ...
    protected function unsetOtherDefaults(ProjectStatus $status): void
    {
        ProjectStatus::query()
            ->where('organization_id', $status->organization_id)
            ->whereKeyNot($status->id)
     ...
     */
    protected function validatedPayload(Organization $organization, array $data, ?ProjectStatus $ignore = null): array
    {
        ...
        if ($isDefault && $isClosed) {
            throw ValidationException::withMessages([
                'is_default' => __('A closed status cannot be set as the default project status.'),
            ]);
        }
